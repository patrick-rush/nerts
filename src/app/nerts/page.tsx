...
export default function Nerts() {
  ...
  const [lastInLake, setLastInLake] = useState<{
    ...
    socket.on(
      ...
      (message: {
        ...
        if (message.lastInLake) {
          ...
          if (movedCard && pileIndex !== -1)
            setLastInLake({ player: movedByPlayer, card: movedCard, pileIndex })
        }
      },
    )
  ...
  const emit = useCallback(function emit<T>(
    socket: Socket,
    event: string,
    arg: T,
  ): void {
    // TODO: set up ack from server to make sure events fire
      ...
      const findTarget = (
        ...
        for (let i = 0; i < repetitions; i++) {
          const values = document
            .getElementById(`${location}-${i}`)
            ?.getBoundingClientRect()
          let offset = 10

          if (location === CardSource.River)
            offset = river[i].length * (height / 5) + 10
          if (
            values &&
